...
namespace nBatonOSC
{
    public partial class Form1 : Form
    {
        ...
        public void SetupControls()
        {
            ...
            groupTransferMode.Font = new Font("MS Gothic", 12);
            //groupTransferMode.Width = 480;
            ...
            radioOff.Font = new Font("MS Gothic", 14);
            radioOff.Name = "0";
            ...
            radioStudio.Text = "1: スタジオ Studio (運営コメント Operator comments)";
            radioStudio.Top = 50;
            radioStudio.Left = 15;
            if (tMode == 1)
            {
            ...
            buttonApply.Location = new Point(10, 120);
            ...
            labelVersionInfo.Location = new Point(130, 25);
            ...
            labelCheckResult.Location = new Point(10, 56);
            ...
            buttonCheckUptates.Location = new Point(10, 24);
            ...
            textURL.Location = new Point(10, 100);
            //textURL.AutoSize = true;  // 12文字程度の短いテキストボックスになる。
            textURL.Width = groupTransferMode.Width - 20;
            textURL.Text = "https://github.com/co1956457/nBatonOSC/releases/latest";
        ...
        private async void ButtonCheckUpdates_Click(object sender, EventArgs e)
        {
            ...
            string checkResultText = "新しいものが公開されています。\nNew version was released.";
            if (_class1.Version == gotVersion)
            {
                checkResultText = "最新版を使っています。\nThis is the latest.";
            }
            else if (gotVersion == "v?.?")
            {
                checkResultText = "情報が取得できませんでした。\nFailed to retrieve information.";
            }
        ...
        /// https://api.github.com/repos/co1956457/nBatonOSC/releases/latest
        /// </summary>
        ...
        private async Task<string> GetLatestVersion()
        {
            ...
            try
            {
                string url = "https://api.github.com/repos/co1956457/nBatonOSC/releases/latest";
                httpClient.DefaultRequestHeaders.TryAddWithoutValidation("User-Agent", "nBatonOSC");
                string responseJson = await httpClient.GetStringAsync(url);
                // Github の API は現在 XML 形式でのレスには対応していないらしい(以前はフォーマット指定ができたらしい)。
                // System.Text.Json, System.Memory, x86 周りの関係か、 NCV 実行環境でエラー(バージョンが合わない？)。
                // 古いバージョンの dll を利用すれば可能かもしれないが、実行環境依存の問題も起きそうなので、
                // 今回は単純に string 操作とした。
                ...
                editJson = editJson.Remove(editJson.IndexOf("\""));                                 // 「"」以降削除
