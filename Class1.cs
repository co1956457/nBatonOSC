...
//
// Compile: CPU Type x86 (参照 Plugin.dll が x86)
//
...
// !!! Add References 参照追加
//     \NiconamaCommentViewer\Plugin.dll, NicoLibrary.dll
using Plugin;
    ...
    public class Class1 : IPlugin
    {
        ...
        void _host_ReceivedComment(object sender, ReceivedCommentEventArgs e)
        {
            if(transferMode > 0) // 稼働中
            {
                ...
                else if (transferMode == 2)// 全転送 (transferMode ==2)
                {
                    // 一般コメントの時
                    if (!isOperator)
                    {
